...
main(int argc, char **argv)
{
	...
	//----------------------
	bool threadlimitFlag = false;
	bool threadStatusFlag = false;
	bool bitmapFlag=false;
//synch
	bool synchFlag=false;
    bool synchrwFlag=false;
    bool synchbrFlag=false;
    int barriernum;
//filesys
    bool fileproFlag=false;

	char *printFileName = NULL; 
    //userprogramme
    bool virtualmemory =NULL;
#ifndef FILESYS_STUB
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    threadlimitFlag = TRUE;
	}
	else if (strcmp(argv[i], "-TS") == 0) {
	    threadStatusFlag = TRUE;
	}
    ...
    else if (strcmp(argv[i], "-sypc") == 0) {
        synchFlag = true;
    }
    else if (strcmp(argv[i], "-syrw") == 0) {
        synchrwFlag = true;
    }
else if (strcmp(argv[i], "-sybr") == 0) {
        synchbrFlag = true;
        //printf("argv:%s\n",argv[i+1]);
        ASSERT(i + 1 < argc);
        barriernum=argv[i+1][0]-'0';
        //printf("barriernum %d\n",barriernum);
    }
	...
	if (threadlimitFlag) {
      kernel->threadLimitTest();  // test threads limits
    }
	if (threadStatusFlag) {
      kernel->threadStatus();  // print threads status
    }
    if (consoleTestFlag) {
    ...
    if(bitmapFlag)
    {
        ...
        //printf("thread 1 code length: 272,datalenth 18,physics page:2\n");
        //printf("thread 2 code length: 346,datalenth 8,physics page:1\n");
    }
 if (synchFlag!=NULL) {
    
      kernel->synchtest();   // two-machine test of the network
    }
    if (synchrwFlag!=NULL) {
    
      kernel->synchrwtest();   // two-machine test of the network
    }
    if(synchbrFlag!=NULL)
    {
         kernel->synchbrtest(barriernum);
    }
// if (printFileName!=NULL) {
//         OpenFile *printfile = kernel->fileSystem->Open(printFileName);
//         //printfile->Print();
//         //printf("length:%d",printfile->Length());
//     }
#ifndef FILESYS_STUB
 ...
 if (virtualmemory!=NULL) {
      ...
      if (space->virtualLoad("../test/test.noff")) {  // load the program into the space
