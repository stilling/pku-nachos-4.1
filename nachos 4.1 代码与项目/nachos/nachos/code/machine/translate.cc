...
Machine::ReadMem(int addr, int size, int *value)
{
    ...
    if(exception==TLBFaultException)
    {
...
Machine::WriteMem(int addr, int size, int value)
{
    ...
    if (exception != NoException) {
	RaiseException(exception, addr);
    if(exception==TLBFaultException||exception==PageFaultException)
    {
        exception=Translate(addr,&physicalAddress,size,FALSE);
    }
	return FALSE;
...
// from the virtual address
	...
	entry = &pageTable[vpn];
        ...
        int t=kernel->stats->totalTicks;
        printf("virtual page %d hit TLB in %d at time:%d\n",vpn,i,t);
		entry = &tlb[i]; 
        entry->times=t;
		break;
	    }
	if (entry == NULL) {				// not found
    	    DEBUG(dbgAddr, "Invalid TLB entry for this virtual page!");
             //printf("TLBFaultException\n");
    	    return TLBFaultException;		// really, this is a TLB fault,
