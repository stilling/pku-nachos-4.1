...
{
	int cout = 0;
	DEBUG('t',"Entering LimitTest");
	for(int i = 0; i <= MaxThreads;i++)
	{
		cout++; 
		Thread *t = new Thread("new thread");
		printf("tname %s userid %d threadid %d \n",t->getName(),t->getUserID(),t->getThreadID());
	}
} 
...
Thread::Print()
{
	...
	cout<<"name:"<<name<<"  tid:"<<ThreadID<<"  status:"<<ThreadStatusChar[status]<<endl;
} 
...
SimpleThread(int which)
{
    ...
    for (num = 0; num < 20; num++) {
        cout << "*** thread " << which <<"_"<<kernel->currentThread->getName()<<" priority="<<kernel->currentThread->getPriority()<< " looped " << num << " times\n";//edit
